...
public class GenerateMatrices {
    ...
    private String filePath = "/home/rachana/Downloads/cloudsim-task-scheduling-master/cloudlets.txt";
    ...
    public GenerateMatrices(List<Vm> vmlist){
        ...
        try {
            if (commFile.exists() && execFile.exists()) {
                ...
                initCostMatrix(filePath);//#####aaaa here it is sending filepath but not recieving
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    {
        @SuppressWarnings("resource")//###why is dis??
        BufferedReader br= new BufferedReader(new InputStreamReader(new FileInputStream(filePath)));
        String data = null;
        int index=0;
        double[] LengthGroup = new double[Constants.NO_OF_TASKS];//##what is it storing??
        while ((data = br.readLine()) != null)
        {
            System.out.println(data);
            String[] taskLength=data.split("\t");//tasklength[i]Is the cost of task execution (number of instructions）
            for(int j=0;j<taskLength.length;j++){
                LengthGroup[index+j] = Double.parseDouble(taskLength[j]);
                if((index+j+1)==Constants.NO_OF_TASKS)
                {
                    br.close();
                    initMatrix(LengthGroup);//raaaaaa  at this time i mean while returning lengthgroup will contain 400 cloudlets or whatever we hv given
                    return;
                }
            }
